...
def run_mll():
    ...
    def recommend_study_method(score):
        ...
        if 10 <= score < 30:
            level = "10-30"
            message = "점수별 팁! : 📖 기초 개념부터 다시 학습, 개념 강의 & 문제 풀이 병행"
        elif 30 <= score < 50:
            level = "30-50"
            message = "점수별 팁! : 📘 자주 틀리는 유형 정리, 쉬운 문제부터 반복 연습"
        elif 50 <= score < 70:
            level = "50-70"
            message = "점수별 팁! : 📝 개념 복습 + 문제 풀이량 증가, 오답 분석 철저히"
        elif 70 <= score < 90:
            level = "70-90"
            message = "점수별 팁! : 🔍 다양한 난이도의 문제 풀이, 시간 관리 연습"
        elif 90 <= score <= 100:
            level = "90-100"
            message = "점수별 팁! : 🎉 실전 모의고사 반복, 취약한 부분 미세 조정"
        else:
